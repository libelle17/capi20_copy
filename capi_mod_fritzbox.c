...
static int fritzboxRemoteCommand( int nHandle, unsigned char *pnBuffer, int nWrite ) {
	...
	if ( nNum == 2 ) {
		...
		nNum = recv( nHandle, pnBuffer, nLen, 0 );
		if ( nNum != nLen ) {
...
static unsigned fritzboxRelease( int nSock, int nApplId ) {
	...
	/* write message to socket */
	if( send( nSock, anMessage, 17, MSG_DONTWAIT ) != 17 ) {
		return 0;
	}

	/* try to read header */
	recv( nSock, anMessage, sizeof( anMessage ), MSG_DONTWAIT );
...
static unsigned fritzboxPutMessage( int nSock, unsigned nApplId, unsigned char *pnMsg ) {
	...
	nNum = send( nSock, anSendBuffer, nLen, 0 );
	if ( nNum != nLen ) {
...
static int readSocket( int nSock, unsigned char *pnBuffer, int nLen ) {
	...
	if ( recv( nSock, anTemp, 3, 0 ) == 3 ) {
		...
		/* read message */
		while ( ( ( nActLen = recv( nSock, &anTemp[ nReadLen ], nTotLen, 0 ) ) < nTotLen ) && ( time( NULL ) < ( nTime + 5 ) ) ) {
			if ( nActLen > 0 ) {
				nTotLen -= nActLen;
				nReadLen += nActLen;
			}
